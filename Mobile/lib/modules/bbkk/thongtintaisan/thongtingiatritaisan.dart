...
class ThongTinGiaTriTaiSan extends StatefulWidget {
  final LoaiBienDongs? taiSan;
  final String? tenDonVi;
  const ThongTinGiaTriTaiSan({Key? key, this.taiSan, this.tenDonVi})
      : super(key: key);

  @override
  _ThongTinGiaTriTaiSanState createState() => _ThongTinGiaTriTaiSanState();
}

class _ThongTinGiaTriTaiSanState extends State<ThongTinGiaTriTaiSan> {
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      child: Scaffold(
        appBar: AccountHeader(subTitle: "Thông tin Giá trị Tài sản"),
        body: SafeArea(
          child: Column(
            children: [
              Expanded(
                          ...
                          child: Column(
                            children: [
                              ...
                              _buildInfoRow(
                                  "Nguyên giá ban đầu",
                                  NumberFormat()
                                      .format(widget.taiSan?.nguyenGiaGoc)
                                      .toString()),
                              SizedBox(height: 12),
                              Divider(color: Colors.grey.shade300, height: 1),
                              SizedBox(height: 12),
                              _buildInfoRow(
                                  "Nguyên giá hiện tại",
                                  NumberFormat()
                                      .format(widget.taiSan?.nguyenGia)
                                      .toString()),
                              SizedBox(height: 12),
                              Divider(color: Colors.grey.shade300, height: 1),
                              SizedBox(height: 12),
                              _buildInfoRow(
                                  "Giá trị còn lại",
                                  NumberFormat()
                                      .format(widget.taiSan?.giaTriConLai)
                                      .toString()),
                              SizedBox(height: 12),
                              Divider(color: Colors.grey.shade300, height: 1),
                              SizedBox(height: 12),
                              _buildInfoRow(
                                  "Luỹ kế HM/KH",
                                  NumberFormat()
                                      .format(widget.taiSan!.haoMonLuyKe! +
                                          widget.taiSan!.khauHaoLuyKe!)
                                      .toString()),
                              SizedBox(height: 12),
                              Divider(color: Colors.grey.shade300, height: 1),
                              SizedBox(height: 12),
